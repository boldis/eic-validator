...
class EICValidationResult(BaseModel):
    ...
    components: Optional[EICComponentsResponse] = Field(
        None,
        description="Parsed EIC components (only if valid)"
    )
...
class EANValidationResponse(BaseModel):
    ...
    format: Optional[str] = Field(
        None,
        description="EAN format (EAN-8, EAN-13, or EAN-14) if valid"
    )
...
class EANGenerationRequest(BaseModel):
    ...
    ean_type: str = Field(
        ...,
        description="Target EAN type",
        examples=["EAN-8", "EAN-13", "EAN-14"]
    )
...
class EANGenerationResponse(BaseModel):
    ...
    generated_ean: str = Field(
        ...,
        description="Generated EAN code with check digit"
    )

    class Config:
        json_schema_extra = {
            "example": {
                "generated_ean": "12345670"
            }
        }
